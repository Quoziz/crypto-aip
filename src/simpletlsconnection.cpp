...
SimpleTLSConnection::SimpleTLSConnection(std::string targetHost, std::string targetPort) noexcept(false)
    : host(std::move(targetHost)),
      port(std::move(targetPort)),
      sslCtx(ssl::context::tlsv13_client),
      stream(this->ioCtx, this->sslCtx)
{
  ...
  {
    beast::error_code errcode{ static_cast<int>(::ERR_get_error()), net::error::get_ssl_category() };
    throw beast::system_error{ errcode, "set SNI hostname" };
  }
