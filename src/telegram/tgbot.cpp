...
void Telegram::TgBot::loadUpdates() noexcept(false)
{
  ...
  if (response->result_int() != 200)
    throwBadStatusError("get updates");
  ...
  try
  {
    ...
    for (const auto& update : content.at("result").as_array())
    {
      ...
      {
        msg->senderId() = std::string(jId->as_string().data(), jId->as_string().size());
      }
      ...
      msg->text() = std::string(jText->as_string().data(), jText->as_string().size());

      if (jPhotoId->is_object() && jPhotoId->at("mime_type").as_string() == "image/bmp")
      {
        std::string fileId;
        fileId = std::string(jPhotoId->at("file_id").as_string().data(), jPhotoId->at("file_id").as_string().size());
...
{
  int res = this->conn
                .sendGet<http::string_body>(this->token + "/sendMessage?chat_id=" + msg->senderId() + "&text=" + msg->text())
                ->result_int();
...
void Telegram::TgBot::sendPhotoMessage(std::shared_ptr<PhotoMessage> msg) noexcept(false)
{
  ...
  if (result != 200)
    throwBadStatusError("send photo");
}
...
void Telegram::TgBot::loadPhoto(const std::string& fileId, boost::asio::streambuf& buffer) noexcept(false)
{
  ...
  if (response->result_int() != 200)
    throwBadStatusError("get photo id");
  ...
  try
  {
    ...
    filePath = std::string(jFilePath.as_string().data(), jFilePath.as_string().size());

    int fileLoadResult = this->conn.loadFile("/file" + this->token + "/" + filePath, buffer);
    if (fileLoadResult != 200)
      throwBadStatusError("load photo");
  }
  catch (const std::exception& ex)
  {
...
{
  this->conn.sendGet<http::string_body>(
      this->token + "/sendMessage?chat_id=" + reciever + "&text=" + INVALID_SYNTAX_MESSAGE);
}
